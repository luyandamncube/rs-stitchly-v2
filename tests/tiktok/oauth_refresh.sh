...
RESPONSE="$OUT_DIR/oauth_refresh_response.json"
...
missing=0
for key in TIKTOK_CLIENT_KEY TIKTOK_CLIENT_SECRET TIKTOK_REFRESH_TOKEN; do
  eval "value=\${$key:-}"
  if [ -z "$value" ]; then
...
if ! command -v python3 >/dev/null 2>&1; then
  echo "python3 is required for token parsing" >&2
  exit 1
fi
...
if "access_token" not in data:
    print("OAuth refresh did not return access_token. Raw response:", file=sys.stderr)
